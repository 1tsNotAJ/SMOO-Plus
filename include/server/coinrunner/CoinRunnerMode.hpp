...
#include "server/coinrunner/CoinRunnerPackets.hpp"
#include "server/gamemode/GameModeBase.hpp"
#include "server/gamemode/GameModeTimer.hpp"

class CoinHintArrow;
class CoinPlayerBlock;
class CoinRunnerIcon;

struct CoinPostProcessingTypes {
...
typedef CoinPostProcessingTypes::Type CoinPostProcessingType;

class CoinRunnerMode : public GameModeBase {
    public:
        // implemented in CoinRunnerMode.cpp:
        CoinRunnerMode(const char* name);
        ...
        void sendCoinPacket(CoinUpdateType updateType);                    // Called instead of Client::sendGameModeInfPacket(), allows setting packet type
        ...
        bool isPlayerCoin() const                      { return mInfo->mIsPlayerCoin;        }
        ...
        // implemented in CoinRunnerModeTrigger.cpp:
        ...
        bool trySetPlayerRunnerState(CoinState state);                // Sets runner to alive or frozen, many safety checks
        ...
        void tryScoreEvent(CoinRunnerPacket* packet, PuppetInfo* other); // Attempt to score when getting a packet that a runner near us was frozen
        bool trySetPostProcessingType(CoinPostProcessingType type);   // Sets the post processing type, also used for disabling
        void warpToRecoveryPoint(al::LiveActor* actor);                 // Warps runner to chaser OR if impossible, last standing position

        // implemented in CoinRunnerModeCam.cpp:
        ...
        // implemented in CoinRunnerModeUtil.cpp:
    ...
    private:
        CoinUpdateType         mNextUpdateType     = CoinUpdateType::PLAYER;             // Set for the sendPacket funtion to know what packet type is sent
        CoinPostProcessingType mPostProcessingType = CoinPostProcessingType::PPDISABLED; // Current post processing mode (snapshot mode)

        GameModeTimer*  mModeTimer  = nullptr; // Generic timer from H&S used for round timer
        CoinRunnerIcon*  mModeLayout = nullptr; // HUD layout (creates sub layout actors for runner and chaser)
        CoinRunnerInfo*  mInfo       = nullptr; // Our own coinrunner status
        al::WipeHolder* mWipeHolder = nullptr; // Pointer set by setWipeHolder on first step of hakoniwaSequence hook

        // Scene actors
        CoinPlayerBlock* mMainPlayerIceBlock = nullptr; // Visual block around player's when frozen
        CoinHintArrow*   mHintArrow          = nullptr; // Arrow that points to nearest runner while being a chaser too far away from runners
