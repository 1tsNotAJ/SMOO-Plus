...
void CoinRunnerConfigMenu::initMenu(const al::LayoutInitInfo& initInfo) {}

const sead::WFixedSafeString<0x200>* CoinRunnerConfigMenu::getStringData() {
    CoinRunnerInfo* freeze = GameModeManager::instance()->getInfo<CoinRunnerInfo>();
    bool isMode = freeze != nullptr && GameModeManager::instance()->isMode(GameMode::COINRUNNER);
...
GameModeConfigMenu::UpdateAction CoinRunnerConfigMenu::updateMenu(int selectIndex) {
    ...
    CoinRunnerInfo* freeze = gmm->getInfo<CoinRunnerInfo>();
    if (!freeze) {
        Logger::log("Unable to Load Mode info!\n");
        return UpdateAction::NOOP;
    }

    bool isMode = gmm->isMode(GameMode::COINRUNNER);
    if (!isMode) {
        return UpdateAction::NOOP;
    }
